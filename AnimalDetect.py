...
while int(trialStatus) != 4:
    ...
    try:
        ...
        if not trialStatus:
            ...
            while int(responseStatus) != 2 and int(responseStatus) != 4:
                ...
                roi_start2 = (0, 0)
                roi_end2 = ((frame.shape[1] // 2), frame2.shape[0])
                ...
                if corners2:
                    ...
                    for corner2, marker_id2 in zip(corners2, marker_ids2):
                        ...
                        # Determine color based on marker orientation and centroid position
                        if -175 < yaw2 < -5 and centroid_x2 < ((frame2.shape[1] // 2)):
                            # Change the border color of the marker to green (animal was not distracted)
                            color2 = (0, 255, 0)
                            distraction = 0
                        else:
    ...
    if corners:
        ...
        for corner, marker_id in zip(corners, marker_ids):
            ...
            # Determine color based on marker orientation and centroid position
            if 5 < yaw < 175 and centroid_x > ((frame.shape[1] // 2)):
                # Change the border color of the marker to violet (animal was not distracted)
                color = (204, 0, 102)
            else:
                # Keep border color of the marker at yellow (animal was distracted)
                color = (255, 0, 255)
