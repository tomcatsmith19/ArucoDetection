...
%% Connect to the MotoTrak Controller.
Clear_Msg(handles.msgbox);                                                  %Clear all messages out of the messagebox.
handles.moto = Connect_MotoTrak('listbox',handles.msgbox);                  %Connect to the Arduino, passing the listbox handle to receive messages.
if isempty(handles.moto)                                                    %If the MotoTrak connection failed...
    close(handles.mainfig);                                                 %Close the GUI.
    clear('run');                                                           %Clear the global run variable from the workspace.
    return                                                                  %Skip execution of the rest of the function.
end
Replace_Msg(handles.msgbox,...
    [datestr(now,13) ' - MotoTrak controller connected.']);                 %Show when the Arduino connection was successful in the messagebox.     
...
%% Check to make sure the vibration module is detected.
temp = handles.moto.device();                                               %Grab the current value of the analog device identifier.
...
str = sprintf('%s - Detected module: %s.',datestr(now,13),...
    upper(handles.device));                                                 %Create a string showing the detected device.
Add_Msg(handles.msgbox,str);                                                %Show the string in the messagebox.


%% Load the loadcell calibration constants.
handles.baseline = handles.moto.get_baseline_float(10);                     %Read in the baseline (unpressed) value for the vibration loadcell.    
handles.slope = handles.moto.get_slope_float(10);                           %Read in the slope (unpressed) value for the vibration loadcell.
fprintf(1,'handles.moto.get_baseline_float(10) = %1.2f\n',...
    handles.baseline);                                                      %Print the baseline to the command line.
fprintf(1,'handles.moto.get_slope_float(10) = %1.2f\n',handles.slope);      %Print the slope to the command line.
if isnan(handles.baseline)                                                  %If the baseline value is NaN...
    handles.baseline = 0;                                                   %Set the baseline to zero.
    handles.moto.set_baseline_float(10,handles.baseline);                   %Save the default baseline value back to the MotoTrak controller.
end
if isnan(handles.slope) || handles.slope == 0                               %If the saved slope is zero...
    handles.slope = 0.5;                                                    %Set the slope value to 0.5.
    handles.moto.set_slope_float(10,handles.slope);                         %Save the default slope value back to the MotoTrak controller.
end


% %% Connect to the OTMB debugger.
% handles.otmb = Connect_OTMB_Debugger('msgbox',handles.msgbox);              %Connect to the OmniTrak Module Bus debugger.
% if isempty(handles.otmb)                                                    %If the OTMB connection failed...
%     StimBehavior_Close(handles.mainfig);                     %Close the program.
%     return                                                                  %Skip execution of the rest of the function.
% end
% Replace_Msg(handles.msgbox,...
%     [datestr(now,13) ' - OTMB debugger connected.']);                       %Show when the Arduino connection was successful in the messagebox.     


% %% Load the LED calibration functions.
% filename = [handles.mainpath handles.computer '_' handles.moto.port ...
%     '_CAROUSEL_ASSIGNMENTS.config'];                                        %Set the expected filename of the configuration file.
% if ~exist(filename,'file')                                                  %If no configuration file was found...
%     StimBehavior_Write_Carousel_Assignments(handles,...
%         filename);                                                          %Create a default carousel assignments file.
% else                                                                        %Otherwise, if at least one configuration file was found...
%     handles = ...
%         StimBehavior_Load_Carousel_Assignments(handles,...
%         filename);                                                          %Call the function to the load the configuration file.
% end
