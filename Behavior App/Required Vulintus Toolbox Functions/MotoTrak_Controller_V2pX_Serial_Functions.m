...
if ~isfield(moto,'version')                                                 %If no version is yet specified...
    ...
    while serialcon.NumBytesAvailable > 0                                      %If there's any junk leftover on the serial line...
        fscanf(serialcon,'%d',serialcon.NumBytesAvailable);                    %Remove all of the replies from the serial line.
    end
    write(serialcon,'Z','uchar');                                          %Send the check status code to the Arduino board.
...
moto.close_serialcon = @()v2p0_close_serialcon(serialcon);                  %Set the function for closing and deleting the serial connection.
...
moto.trigger_feeder = @()v2p0_simple_command(serialcon,s.TRIGGER_FEEDER);   %Set the function for sending a trigger to a feeder.
...
output = sscanf(temp,'%d')';                                                %Convert the text to numbers.
...
while serialcon.NumBytesAvailable < 1 && now < timeout                      %Loop until there's a reply or the operating times out.
    pause(0.001);                                                           %Pause for 1 millisecond.
end
if serialcon.NumBytesAvailable >= 1                                         %If the controller replied...
    output = read(serialcon,1,'uint8');                                     %Read the reply from the serial line as an unsigned 16-bit integer.
end
...
if serialcon.NumBytesAvailable >= 2                                         %If the controller replied...
    output = read(serialcon,1,'int16');                                     %Read the reply from the serial line as an unsigned 16-bit integer.
end
...
write(serialcon,cmd,'uint8');                                               %Send the command to the controller.
write(serialcon,int,'int16');                                               %Send the int16 argument.
for i = 1:dummy_bytes                                                       %Step through any dummy bytes.
    write(serialcon,0,'uint8');                                             %Send a dummy byte to advance the command queue.
end
...
write(serialcon,cmd,'uint8');                                               %Send the command to the controller.
write(serialcon,int,'uint8');                                               %Send the uint8 value to the controller.
write(serialcon,0,'uint16');                                                %Send a dummy uint16 value to the controller.
output = [];                                                                %Create a variable to hold the output.
timeout = now + 1/86400;                                                    %Set the reply timeout duration (100 milliseconds).
while serialcon.NumBytesAvailable < 2 && now < timeout                      %Loop until there's a reply or the operating times out.
    pause(0.001);                                                           %Pause for 1 millisecond.
...
write(serialcon,s.SET_STREAM_ORDER,'uint8');                                %Send the command to the controller.
write(serialcon,stream_order,'uint8');                                      %Send the modified stream order back to the controller.
...
write(serialcon,s.RETURN_STREAM_ORDER,'uint8');                             %Send the command to the controller.
output = [];                                                                %Create a variable to hold the output.
timeout = now + 1/86400;                                                    %Set the reply timeout duration (100 milliseconds).
while serialcon.NumBytesAvailable < 6 && now < timeout                      %Loop until there's a reply or the operating times out.
    pause(0.001);                                                           %Pause for 1 millisecond.
end
if serialcon.NumBytesAvailable >= 6                                         %If the controller replied...
    stream_order = read(serialcon,6,'uint8');                               %Read the reply from the serial line as unsigned 8-bit integer.
...
% N = fix(serialcon.NumBytesAvailable/ln_bytes);                                 %Check how many lines are available.
...
%         output(i,1) = read(serialcon,1,'uint32');                          %Read in the sample timestamp.
%         for j = 1:num_inputs                                                %Step through the streaming inputs.
%             output(i,j+1) = read(serialcon,1,'int16');                     %Read in each input sample.
%         end
%     end
% end
...
write(serialcon,s.SET_CAGE_LIGHTS,'uint8');                                 %Send the command to the controller.
write(serialcon,pwm_val,'uint8');                                           %Send the PWM value to the controller.


%% This function closes the serial connection and deletes the serial object.
function v2p0_close_serialcon(serialcon)
flush(serialcon);                                                           %Clear the input and output buffers.
